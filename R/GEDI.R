...
#' is to map the reporter IDs to gene IDs. The second step is to collapse
#' rows mapped to the same gene ID. The third step is to merge the
...
#'      mapTo = c("ensembl_gene_id","entrezgene_id"),
...
#' If \code{FALSE}, the \code{GEDI} function maps reporter IDs to gene IDs
...
#' Ensembl or Entrez gene IDs. The second columns name must correspond to the BioMart attribute,
#' e.g. \dQuote{ensembl_gene_id}.
#'
#' @param mapTo (optional) Character string. Which gene ID to map reporter IDs to.
#' Must be either \code{"ensembl_gene_id"} (default) or \code{"entrezgene_id"}.
#'
...
#'
#' @details
#' The \code{GEDI} function maps reporter IDs of the datasets to Ensembl or Entrez gene IDs
...
#' or Entrez gene IDs, then it must contain another type of reporter ID with a known BioMart
#' attribute.
...
#' not to use BioMart.
#'
#' Multiple reporter IDs can map to the same gene ID. Therefore, these
...
#' predicted for all gene IDs where the percentage of known expression
...
GEDI <- function(datasets, attributes,
  ...
  ## BioMart ##
  if (BioMart==TRUE & any(attributes != mapTo)){
  ...
  for (i in 1:length(datasets)){
    ...
    ID <- attributes[i]
    if (.NAtoFALSE(ID != mapTo) | is.na(ID)){
...
#### .GEDI.checkInput ####
.GEDI.checkInput <- function(datasets, attributes, species, BioMart, mapTo, collapseMethod,
  ...
  ## mapTo ##
  validMapTo <- c("ensembl_gene_id","entrezgene_id")
  if (!mapTo %in% validMapTo){
    n.mapTo = length(validMapTo)
    mapTo.msg = paste(paste(dQuote(validMapTo[-n.mapTo]),collapse=", "),
                               "and",dQuote(validMapTo[n.mapTo]),sep=" ")
    stop(gettextf("Invalid 'mapTo' chosen. Valid options are: %s",mapTo.msg))
  }
...
.getAnnot <- function(Data, ID, Data.name,
  ...
  if (BioMart){
    if (.NAtoFALSE(ID %in% BM_attributes(species)$name)){
      ...
      genes <- rownames(Data)
      annot <- getBM(filters= ID, attributes= c(mapTo, ID),values=genes,mart= mart)
  ...
  if (.NAtoFALSE(ID2 != mapTo) | is.na(ID2)){
    if (BioMart){
      if (.NAtoFALSE(ID2 %in% BM_attributes(species)$name)){
        ...
        genes <- annot[,ID2]
        G_list <- getBM(filters= ID2, attributes= c(mapTo, ID2),values=genes,mart= mart)
      ...
      progress.bar$message(paste("Second column of annotation file is not '", mapTo,
                                 "'. Change annotation file or set BioMart=TRUE",sep=""))
      return(NULL)
    }
  }
  return(annot)
...
.canFill <- function(row,threshold){
  n <- length(row)
  foo <- sum(!is.na(row))
  return(foo < n & foo > threshold*n)
}
